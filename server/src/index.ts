...
import { restoreDbFromSupabase, backupDbToSupabase, startSupabasePeriodicBackup, stopSupabasePeriodicBackup } from './db/supabaseBackup';
...
const onListen = () => {
  ...
  // Start periodic backup to BOTH MongoDB and Supabase every 5 minutes
  startPeriodicBackup(5 * 60 * 1000);
  startSupabasePeriodicBackup(5 * 60 * 1000);
...
async function bootstrap(): Promise<void> {
  // Restore SQLite DB from Supabase first (primary), fall back to MongoDB.
  // This ensures container restarts always boot with the last persisted state.
  const restoredFromSupabase = await restoreDbFromSupabase();
...
function shutdown(signal: string): void {
  ...
  server.close(() => {
    ...
    closeDb();
    // Final backup to BOTH Supabase and MongoDB before exit
    Promise.all([backupDbToSupabase(), backupDbToMongo()]).then(() => {
      sLogInfo('Shutdown complete');
      process.exit(0);
    }).catch(() => {
      sLogInfo('Shutdown complete (backup partial)');
      process.exit(0);
    });
  });
  setTimeout(() => {
